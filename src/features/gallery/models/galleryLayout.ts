...
const getPhotoAspectRatio = (photo: GalleryLayoutPhotoLike) => {
...
const getCardHeight = (photo: GalleryLayoutPhotoLike, columnWidth: number) => {
  const aspectRatio = getPhotoAspectRatio(photo);
  const rawHeight = columnWidth / Math.max(0.2, aspectRatio);
  return Math.max(
    GALLERY_MIN_CARD_HEIGHT,
    Math.min(GALLERY_MAX_CARD_HEIGHT, Math.round(rawHeight)),
  );
};
